  ...
  try {
    ...
    if (rawTemplateRow && !isMessageTemplate(rawTemplateRow)) {
      return NextResponse.json(
        {
          error:
            'O registro do template está malformado localmente. Use "Sincronizar da Meta" em Configurações para reparar antes de transmitir.',
        },
        { status: 500 },
      )
    }
    ...
    for (const recipient of recipients) {
      ...
      for (const variant of variants) {
        try {
          ...
          const errorMessage =
            error instanceof Error ? error.message : 'Desconhecido error'
      ...
      if (sentMessageId) {
        ...
        results.push({
          phone: recipient.phone,
          status: 'failed',
          error: lastError || 'Desconhecido error',
        })
        failedCount++
      }
    }
