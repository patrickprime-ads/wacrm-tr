  ...
  try {
    ...
    if (payload.category === 'Authentication') {
      return NextResponse.json(
        {
          error:
            'Templates de AUTENTICAÇÃO ainda não são suportados aqui. Crie-os no Meta WhatsApp Manager e use "Sincronizar da Meta".',
        },
        { status: 400 },
      )
    }
    ...
    if (upsertErr) {
      ...
      return NextResponse.json(
        {
          error: `Enviado à Meta, mas houve falha ao salvar localmente: ${upsertErr.message}. Use "Sincronizar da Meta" para recuperar.`,
