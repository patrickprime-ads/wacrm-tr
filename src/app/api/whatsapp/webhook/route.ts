...
import { dispatchInboundToFluxos } from '@/lib/flows/engine'
import {
...
interface WhatsAppMessage {
   ...
   * we put on the button/row when sending — the Fluxos engine users this
...
async function processMessage(
  ...
  // ============================================================
  const flowResult = await dispatchInboundToFluxos({
    accountId,
...
async function parseMessageContent(
   ...
   * Fluxos engine to advance the per-contact run; persisted to
  ...
  switch (message.type) {
    ...
    case 'interactive': {
      ...
      // stable id separately so the Fluxos engine can route on it.
      const reply =
