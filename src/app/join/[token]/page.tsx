...
export default function JoinPage() {
  ...
  // strict mode dev is harmless. We also user? the `cancelled` flag
  ...
  const handleAccept = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // a clear next-action (sign out → user? different email)
        ...
        <Card className="w-full max-w-md border-border bg-card">
          ...
          <CardContent className="flex flex-col gap-3">
            ...
            <p className="text-center text-xs text-muted-foreground">
              ...
              <span className="text-muted-foreground">{peek.account_name}</span>. Sua conta pessoal vazia criada no cadastro será removida.
            </p>
          </CardContent>
        </Card>
          ...
          <DialogContent className="bg-popover border-border sm:max-w-md">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2 text-popover-foreground">
                ...
                Não é possível entrar {peek.account_name} com esta conta
              </DialogTitle>
            ...
            <div className="space-y-2 py-2 text-xs text-muted-foreground">
              <p>
                Para entrar em{' '}
                <span className="text-popover-foreground">{peek.account_name}</span>, saia e cadastre-se novamente com outro endereço de e-mail. O link do convite continua válido enquanto não expirar.
              </p>
            </div>
            <DialogFooter className="bg-popover border-border">
              ...
              >
                {signingOut ? (
                  ...
                  'Sign out & user? a different email'
                )}
              </Button>
            </DialogFooter>
          </DialogContent>
