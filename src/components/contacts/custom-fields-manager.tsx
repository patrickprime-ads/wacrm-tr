...
/**
 * Dialog wrapper around {@link CustomFieldsPanel}, used on the Contatos page.
...
export function CustomFieldsPanel() {
  ...
  async function handleDelete(field: CustomField) {
    if (
      !window.confirm(
        `Excluir "${field.field_name}"? Isso também remove o valor salvo em todos os contatos. Esta ação não pode ser desfeita.`
      )
    ) {
      return;
    }
