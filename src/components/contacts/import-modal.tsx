...
export function ImportModal({
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (rows.length === 0) {
      toast.error(
        'Nenhuma linha v?lida encontrada. Verifique se o CSV tem uma coluna "phone".'
      );
      setParsedRows([]);
      setHasTagsColumn(false);
      setHasCompanyColumn(false);
  ...
  async function handleImport() {
    ...
    try {
      ...
      // 2) Skip numbers already in this account. One read de the
      ...
      // 4) Batch insert the genuinely-new rows in chunks de 50. The DB
      ...
      try {
        ...
        toast.warning('Contatos imported, but some tag assignments failed.');
      }
      ...
      if (skippedNames.length > 0) {
        ...
        toast.info(
          `Desconhecido tags skipped (create them in Settings first): ${sample}${more}`
        );
      }
      if (skipped > 0) {
        ...
        <div className="shrink-0 space-y-4 border-b border-border/80 px-6 pt-6 pb-5">
          <DialogHeader className="gap-1.5">
            ...
            <DialogDescription className="leading-relaxed text-muted-foreground">
              Envie um CSV com a coluna obrigat?ria{' '}
              ...
              (separadas por vírgula; user? aspas em células com múltiplas tags).
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-3">
              ...
              {parsedRows.length > PREVIEW_LIMIT && (
                <p className="text-center text-[11px] text-muted-foreground">
                  + {parsedRows.length - PREVIEW_LIMIT} linha(s) a mais
                  {parsedRows.length - PREVIEW_LIMIT !== 1 ? 's' : ''} não exibida(s)
                </p>
              )}
            </div>
          )}
              ...
              <div className="mt-3 flex flex-wrap gap-3">
                ...
                {result.tagsAssigned > 0 && (
                  <div className="flex items-center gap-1.5 text-sm text-cyan-400">
                    ...
                    {result.tagsAssigned !== 1 ? 's' : ''} atribuída(s)
                  </div>
                )}
                {result.skipped > 0 && (
