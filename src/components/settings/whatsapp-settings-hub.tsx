...
import { QrCode, ShieldCheck, MessagesSquare } from "lucide-react";
import { EvolutionConfig } from "./evolution-config";
import { WhatsAppConfig } from "./whatsapp-config";
import { ZernioConfig } from "./zernio-config";

export function WhatsAppSettingsHub() {
  const params = useSearchParams();
  const requested = params.get("provider");
  const [provider, setProvider] = useState<"zernio" | "evolution" | "meta">(requested === "meta" || requested === "evolution" ? requested : "zernio");
  return <section className="space-y-5">
    ...
    <div className="grid gap-3 lg:grid-cols-3">
      <button type="button" onClick={() => setProvider("zernio")} className={`rounded-xl border p-4 text-left transition-colors ${provider === "zernio" ? "border-cyan-500/50 bg-cyan-500/10" : "bg-card hover:bg-muted/50"}`}><MessagesSquare className="h-5 w-5 text-cyan-400" /><strong className="mt-2 block text-sm">Zernio — oficial</strong><span className="text-xs text-muted-foreground">WhatsApp, Instagram e Messenger</span></button>
    ...
    {provider === "zernio" ? <ZernioConfig /> : provider === "evolution" ? <EvolutionConfig /> : <WhatsAppConfig />}
  </section>;
}
