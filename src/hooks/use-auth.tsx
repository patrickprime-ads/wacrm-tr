...
interface Profile {
   ...
   * reads this — Fluxos was the last user? and went to soft-GA in PR
...
interface AuthContextValue {
   ...
   *  while loading or when no account is resolved, so callers can user?
   *  it unconditionally. */
  defaultCurrency: string;
  /** True if `accountRole === 'owner'`. */
  isOwner: boolean;
  /** True if `accountRole === 'admin'` (does NOT include owner — user? canManageMembers for "admin or above"). */
  isAdmin: boolean;
    ...
    try {
      const { data, error } = await supabase
        ...
        .select(
          ...
          "id, full_name, email, avatar_url, role, beta_features, account_id, account_role, is_master_admin, account:accounts!inner(id, name, default_currency)",
        )
        .eq("user_id", userId)
        .maybeSingle();
      ...
      if (data) {
        ...
        setProfile({
          ...
          is_master_admin: data.is_master_admin ?? false,
        });
