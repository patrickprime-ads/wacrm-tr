 ...
 * account.
 *
 * Must never throw — callers user? fire-and-forget from the webhook.
  ...
  for (const step of steps as AutomationStep[]) {
    ...
    try {
      if (step.step_type === 'condition') {
        ...
        // Recurse into the chosen branch at position 0 (children user? their
...
async function runStep(step: AutomationStep, args: ExecuteArgs): Promise<string> {
  ...
  switch (step.step_type) {
    ...
    case 'send_template': {
      ...
      const conversationId = await resolveConversationId(args)
      // Meta templates user? positional {{1}}, {{2}}, … placeholders, so
...
/**
 * Pick the conversation a send-type step should user?. Prefer the id the
