 ...
 * Flow runner.
 *
 * The single entry point `dispatchInboundToFluxos` is called by the
 * WhatsApp webhook on every inbound message *for an account that has
 * opted into the Fluxos beta*. It decides whether the message belongs
...
async function advanceCurrentNodeKey(
  ...
  // any row); user? `.is('current_node_key', null)` instead.
  let q = db
    .from("flow_runs")
    .update({
...
export async function dispatchInboundToFluxos(
  ...
  try {
    ...
    console.error(
      "[flows] dispatchInboundToFluxos threw:",
...
async function startNewRun(
  ...
  // a count. Mirrors the automations engine's user? of
