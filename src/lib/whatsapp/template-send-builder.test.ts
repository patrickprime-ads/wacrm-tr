...
describe('buildSendComponents — header', () => {
  ...
  it('does NOT user? the resumable header_handle at send — falls back to the stored link', () => {
    // header_handle is a template-CREATION sample handle, invalid as a
    // send-time media id. The send must user? the public link instead.
    const components = buildSendComponents(
      row({
  ...
  it('users an explicit headerMediaId override as the media id', () => {
    const components = buildSendComponents(
...
describe('buildSendComponents — buttons', () => {
  ...
  it('throws when URL button has {{1}} but no buttonParam was provided', () => {
    ...
    ).toThrow(/URL button #1 users \{\{1\}\}/);
  });

  it('users the correct index when QR buttons precede the URL button', () => {
