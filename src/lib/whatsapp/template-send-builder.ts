 ...
 *     override — that matches the most common user? case (a static
  ...
  switch (button.type) {
    case 'URL': {
      ...
      if (!override || !override.trim()) {
        throw new Error(
          `URL button #${index + 1} users {{1}} — requires a buttonParams[${index}] value.`,
        );
      }
      return {
        type: 'button',
    ...
    case 'QUICK_REPLY': {
      ...
      // payload (rare — usually QR buttons user? their default text).
      return {
        type: 'button',
