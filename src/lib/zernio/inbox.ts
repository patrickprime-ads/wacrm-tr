...
export async function importZernioConversation(db: SupabaseClient, accountId: string, ownerId: string, apiKey: string, remote: ZernioConversation) {
  ...
  // Preserve the customer's actual first message time. Historical imports
  // must not turn old leads into "leads today" merely because sync ran now.
  ...
  const firstInboundAt = messages
    .filter((message) => message.direction === "incoming" && message.createdAt)
  ...
  if (!contact) {
    const inserted = await db.from("contacts").insert({ account_id: accountId, user_id: ownerId, phone, name: remote.participantName || (remote.platform === "whatsapp" ? phone : remote.participantId), avatar_url: remote.participantPicture || null, lead_source: remote.platform, lead_temperature: "frio", ...(firstInboundAt ? { created_at: firstInboundAt } : {}) }).select("id, name, avatar_url").single();
  ...
  if (!local) {
    const inserted = await db.from("conversations").insert({ account_id: accountId, user_id: ownerId, contact_id: contact.id, channel: remote.platform, external_provider: "zernio", external_conversation_id: remote.id, external_account_id: remote.accountId, last_message_text: remote.lastMessage || null, last_message_at: remote.updatedTime || new Date().toISOString(), unread_count: remote.unreadCount || 0, ...(firstInboundAt ? { created_at: firstInboundAt } : {}) }).select("id").single();
    if (inserted.error) throw inserted.error;
    local = inserted.data;
  } else {
